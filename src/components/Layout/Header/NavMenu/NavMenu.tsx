...
export default function NavMenu() {
  const [markMenu, setMarkMenu] = useState<boolean[]>([true, false, false, false, false]);
  useEffect(() => {
    inView('section', ({target}) => {
      switch (target.id) {
        ...
        case 'ABOUT':
          ...
          break;
        case 'CONTACT':
          setMarkMenu([false, false, false, false, true]);
          break;
        default:
          setMarkMenu([false, false, false, false, false]);
          break;
      }
    })
  })
    ...
    <nav className={style.navmenu}>
      <ul>
        ...
        >
          <a
            href="#HOME"
            onClick={ () => setMarkMenu([true, false, false, false, false]) }
        ...
        <li className={`${style.link} ${markMenu[4] ? style.mark : ''}`}>
          <a
            href="#CONTACT"
            onClick={ () => setMarkMenu([false, false, false, false, true]) }
          >
            Contato
          </a>
        </li>
